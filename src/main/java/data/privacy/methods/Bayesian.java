...
public class Bayesian implements CountingQuery, ContingencyTable {
	...
	public Bayesian(Random rng1, Data data1, double ep, double alloc, double theta, int kbound) {
		...
		//判断实际满足theta-usefulness的最大K值
		while (k > 0){
			double num1 = ep * (1-alloc) * data.getNum();   //LEMMA 3
		...
		InjectNoise(ep * (1-alloc), model);							//添加laplace噪声，得到cCache_Noisy
		intSyn = Sampling(data.getNum(), model);					//sampling  二进制
	 ...
	 * Training the Bayesian Network.
	 * Initialize
	 *   1. S = empty set of integer.      已经找到依赖关系的属性集
	 *   2. V = the array of attributes.   属性集
	...
	private HashSet<Dependence> S2V(HashSet<Integer> S, HashSet<Integer> V, int k) {
		...
		HashSet<HashSet<Integer>> kS = GenTool.kSub(S, k);  //在S集中查找所有的k-way子集合

		if (kS.isEmpty()) kS.add(new HashSet<Integer>(S));
		
		// the Cartesian product of kS and V.  笛卡尔积
