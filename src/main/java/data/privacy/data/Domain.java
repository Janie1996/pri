...
public class Domain {
	...
	// a map of attributes with @Translator.  保存属性及对应的候选值
	...
	// the array of bin(grid) numbers of a(an) column(attribute).  各个属性有几个候选值个数
	private int[] cells;
	...
	public Domain(String dataDomain) throws Exception {
		...
		// read the "*.domain" from file system.   读入的属性集  eg：年龄（1 2 3） 学历（学士 硕士 博士）
		BufferedReader domainFile = new BufferedReader(new FileReader(dataDomain));
		
		// specifies the dimension of data.  指定数据集维度
